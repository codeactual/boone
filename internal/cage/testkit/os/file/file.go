...
import (
	...
	tp_file "github.com/codeactual/boone/internal/third_party/stackexchange/os/file"
...
// CreateDir creates a file under DynamicDataDir() at a relative path built by joining the path parts.
//
// All intermediate directories are created if needed. All directories are created with 0700.
func CreateDir(t *testing.T, pathPart ...string) (relPath string, absPath string) {
	relPath, absPath = CreatePath(t, pathPart...)

	err := os.MkdirAll(absPath, 0700)
	testkit.FatalErrf(t, err, "failed to make dir [%s]", absPath)
