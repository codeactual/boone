...
pkg-path-base = github.com/codeactual/boone
...
test:
	@which testecho > /dev/null || echo "testecho must be installed for tests: make test-dep"
	@mkdir -p ./testdata/cover
	@CGO_ENABLED=1 go test -race -coverprofile=./testdata/cover/cover.out ./internal/boone ./internal/cage/...
